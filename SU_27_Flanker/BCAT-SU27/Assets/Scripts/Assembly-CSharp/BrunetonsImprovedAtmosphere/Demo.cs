...
public class Demo : MonoBehaviour
{
	...
	private void Initialize()
	{
		...
		for (int i = num; i <= num2; i += 10)
		{
			...
			double num11 = num9 / num7 * Math.Pow(x, 0.0 - num8);
			list2.Add(i);
		...
		m_model.MiePhaseFunctionG = miePhaseFunctionG;
	...
	{
		Shader.SetGlobalVector("_ScattEarthCenter", new Vector3(0f, (0f - kBottomRadius) / kLengthUnitInMeters, 0f));
		Shader.SetGlobalVector("_ScattSunSize", new Vector2(Mathf.Tan(kSunAngularRadius), Mathf.Cos(kSunAngularRadius)));
		Shader.SetGlobalVector("_ScattSunDir", ((Sun == null) ? Vector3.up : Sun.transform.forward) * -1f);
	}
}}
